...
LEFT JOIN individual_lesson as i ON ap.id =i.appointment_id 
LEFT JOIN audition as aud ON ap.id=aud.appointment_id
WHERE EXTRACT(YEAR FROM date) = '2020'
GROUP BY EXTRACT(MONTH FROM date)
...
GROUP BY instructor, EXTRACT(DAY FROM a.date)
